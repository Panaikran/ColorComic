...
class RecentOutputsUiTests(unittest.TestCase):
    ...
    def test_processing_page_surfaces_progress_and_error_state_clearly(self):
        ...
        self.assertIn('id="cpuProcessingNote"', template)
        self.assertIn("Running on CPU. Large PDFs may take several minutes.", template)
        self.assertIn('{% if job.device | lower == "cpu" %}', template)
        ...
        self.assertIn("function hideProcessingActive()", template)
        self.assertIn("if (cpuProcessingNote)", template)
        self.assertEqual(template.count("hideProcessingActive();"), 2)
